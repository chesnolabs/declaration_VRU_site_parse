...
def get_people (url):
    ...
    page = get_page(url)
    if page!=None:
        people_re = re.compile('<a href.*blank.*a>',re.IGNORECASE)
        strings = people_re.findall(page)
        print 'Total links found: ',len(strings)
    else:
        print "Can't get a list of deputies after " + str(TRIES) + " attempt(s). Possible, www.rada.gov.ua is not callable"
        return {}
    re_name = re.compile('>.*<',re.IGNORECASE)
    re_href = re.compile('http.*\d"',re.IGNORECASE)
    for string in strings:
        href = re_href.findall(string)[0][:-1]
        deputy_name = re_name.findall(string)[0][1:-1]
        list_of_dep[get_person_id(href)] = [href, unicode(deputy_name,'cp1251')]
    return list_of_dep
